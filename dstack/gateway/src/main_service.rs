...
impl ProxyInner {
    ...
    pub async fn new(
        ...
        // This node's own records are written *after* the bootstrap below, not here.
        // A local write allocates a sequence number, and after a data-directory loss
        // this node has no record of which numbers it already spent — only its peers
        // do. `bootstrap` rebuilds the counter from their coverage, so anything written
        // before it reuses numbers the peers already treat as seen and is silently
        // dropped cluster-wide. That would strand exactly the records recovery depends
        // on: the fresh uuid peers check us against, and our sync address.
