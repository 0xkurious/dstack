...
impl HttpSyncNetwork {
    ...
    ///
    /// Our own uuid is local configuration, not replicated state, and sourcing it from
    /// the store forced this node's `node/info` record to be written before the service
    /// could be built — which is to say before `bootstrap` had rebuilt the sequence
    /// counter. After a data-directory loss that made the record spend a sequence number
    /// the peers already consider seen, so the one record they check us against was the
